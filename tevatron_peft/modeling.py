...
class DenseModel(nn.Module):
    ...
    def encode_passage(self, psg):
        ...
        psg_out = self.lm_p.base_model(**psg, return_dict=True)
    ...
    def encode_query(self, qry):
        if qry is None:
            return None, None
        qry_out = self.lm_q.base_model(**qry, return_dict=True)
    ...
    def save(self, output_dir: str):
        if self.model_args.untie_encoder:
            ...
            self.lm_p.save_pretrained(output_dir)
        else:
    ...
    def build(
        ...
        if os.path.isdir(model_name_or_path):
            ...
            if os.path.exists(_qry_model_path):
                ...
                #import pdb;pdb.set_trace()
                from peft import PeftModel, PeftConfig
                config = PeftConfig.from_pretrained(_qry_model_path)
                model = AutoModel.from_pretrained(config.base_model_name_or_path)
                model = PeftModel.from_pretrained(model, _qry_model_path)
                lm_q = model
                ...
                logger.info(f'loading passage model weight from {_psg_model_path}')
                from peft import PeftModel, PeftConfig
                config = PeftConfig.from_pretrained(_psg_model_path)
                model = AutoModel.from_pretrained(config.base_model_name_or_path)
                model = PeftModel.from_pretrained(model, _psg_model_path)
                lm_p = model
                """
                lm_p = AutoModel.from_pretrained(
                    _psg_model_path,
